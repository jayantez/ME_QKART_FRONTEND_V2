...
const Checkout = () => {
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      // console.log('Response:88', response.data);
      setAddresses({ ...addresses, all: response.data });
      return response.data;
    } catch (e) {
      if (e.response) {
  ...
  //   console.log("address state out");
  //   setAddresses({...addresses, selected: addressId });
  //   //console.log(addresses,"address state out");
  // }
  ...
  useEffect(() => {
    ...
    //console.log(addresses,"address state");
    onLoadHandler();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  console.log(addresses,"address state");
  const selectedAddress = addresses.selected;
  const addressesList = [...addresses.all];
  const isNewAddress = newAddress.isAddingNewAddress
 console.log(isNewAddress,"cccc");
  return (
    <>
      <Header />
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
             ...
             { (addresses.all == []) && <Typography my="1rem" className = "address-item" >
                 No addresses found for this account. Please add one to proceed
               </Typography> }

               { addressesList.map((address)=>(           
               <Typography my="1rem"  key ={address._id} className = {(selectedAddress === address._id ) ? "address-item selected" : "address-item not-selected"} onClick = {()=>{setAddresses({...addresses, selected: address._id })}} >
                {address.address}
           ...
           {!isNewAddress && <Button
            ...
            {isNewAddress && <AddNewAddressView
                token={token}
